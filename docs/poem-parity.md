...
| ~~gRPC対応~~ | ★☆☆ | ✅ 完了(2026-07-12)。`grpc.health.v1.Health/Check`(unary)・`/Watch`(streaming)。他サービス・リフレクションは未対応 |
...
チャレンジが完了(2026-07-12)。gRPCもストリーミング(`Health/Watch`)を
同日中に追加し、unary(`Check`)とストリーミング両方をカバー(2026-07-12)。
残るのは各機能の対応範囲拡大(Multipart以外のファイル添付、DNS-01
チャレンジ(実DNSプロバイダAPI連携が必要、この開発環境では現実的な
検証手段が無い)、gRPCの他サービス・リフレクション等)のみ。
