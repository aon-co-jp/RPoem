...
RustJSON は、このエコシステム向けに設計した、人間が書きやすい JSON の
上位互換フォーマットです。**新しいデータモデルは導入しません**——
パース結果は標準の `serde_json::Value` そのものであり、このワーク
スペースの他のあらゆる JSON 消費コードがそのまま扱えます。RustJSON が
...
正規化されます——RustJSON が JSON にできない新しい意味表現を持つことは
...
完全な自前実装(`crates/open-runo-rustjson`)——このワークスペースが
...
- `crates/open-runo-rustjson`: パーサー+シリアライザ本体。
  `parse(&str) -> Result<serde_json::Value, RustJsonError>`・
  `to_string(&Value) -> String`(常に厳密な JSON を出力——RustJSON の
  緩さは入力側のみの利便性で、保存・送信される正規形は常に曖昧さの
  無い標準 JSON)。17件のユニットテストで4拡張すべて+エラーケース
  (未終端文字列・未終端コメント・不正キー等)を検証済み。
- `crates/open-runo-db`(`rustjson` feature): `RustJsonBackend`——
  `DbBackend` trait実装。`put`時に`open_runo_rustjson::parse`で検証し、
...
- `open_runo_rustjson::extract_path(&Value, path: &str) -> Option<&Value>`
...
- **未完了(次回セッションの最初のタスク)**: `RustJsonBackend`側での
  ...
  `open-runo-rustjson`クレート自体の`extract_path`は完成・テスト済みで
  すぐ使える状態——残るのは配線のみ。
...
`RustJsonBackend`は`DbBackend` traitを実装しているため、将来的に
永続化・分散化されたRustJSON専用エンジンに差し替える際も、呼び出し側
(ハンドラ等)のコード変更は不要な設計になっています。

## デメリットへの対応方針(ご指摘の4点)

1. **独自パース処理の限界(RDBへの正規化の難しさ)**: `RustJsonBackend`は
   RDBではなくKVストア形状(`(table,key)→value`)のため、正規化の
   問題自体が今のところ発生しません。将来的なテーブル横断クエリ・
   結合が必要になった時点で再検討。
2. **ロックイン(特定DBへの依存)**: RustJSON自体は`serde_json::Value`を
   値モデルとするため、PostgreSQLのJSONB型固有の機能には一切依存
   していません(`open-runo-rustjson`クレートはDB非依存の純粋なパーサー
   /シリアライザ)。
3. **シリアライズのオーバーヘッド**: `to_string`は`serde_json`の
   コンパクトシリアライザをそのまま利用しており、RustJSON独自のシリア
...
4. **ORMエコシステムとの乖離**: このワークスペースは元々Diesel/SeaORM
   ...
   はREST API経由でアクセスする設計であり、RustJSON独自型をORMのRust
   マクロに認識させる必要がありません。
