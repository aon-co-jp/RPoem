...
- 上記2026-07-19の要望に基づき、実際に`cargo test --workspace`を実行して
  ...
  - **今回のスコープ外**: 上記2件の実修正は行っていない(発見・原因
    特定・記録のみ)。`cargo test --workspace`のうち上記2クレート以外の
    結果、および`open-runo-router`単体テストの結果は、コンテキスト
    ウインドウ制限により今回のセッションでは確認しきれなかった
    (次回セッションでまず`cargo test --workspace 2>&1 | grep -E
