...
- **2026-07-14 RJSON Phase 2 着手(サーバー側部分抽出のコア実装完了) —
  ユーザー指示により本日はここで停止・ドキュメント整理してpush**:
  `crates/open-runo-rjson/src/lib.rs`に`extract_path(&Value, path:
  &str) -> Option<&Value>`(ドット/ブラケット記法のパス言語、
  `stats.damage`・`bonuses[1]`・`items[2].name`)を追加、外部パース
  クレート非依存の自前実装(`parse_path_segments`)。8件のユニット
  テスト追加(`open-runo-rjson`は17→25テストに、全greenを確認済み)。
  `crates/open-runo-db/src/lib.rs`の`DbBackend` traitに`get_field(table,
  key, path) -> Result<Option<String>>`のデフォルト実装(未対応の
  backendには`AppError::Validation`を返す)を追加。`docs/rjson.md`に
  「実装(Phase 2、着手済み)」節を追記。open-runo側へも
  `open-runo-rjson`クレート・`get_field`トレイトデフォルト・
  `docs/rjson.md`をミラー、`cargo test -p open-runo-rjson`(25件)・
  `cargo test -p open-runo-db --features rjson rjson`(4件)を両リポジトリ
  で確認済み。
  **明示的に未着手のまま次回セッションへ引き継ぐタスク(ユーザー指示
  「これとこれを行なう」への回答)**:
  1. `RjsonBackend`(`crates/open-runo-db/src/lib.rs`の
     `rjson_backend`モジュール)側で`get_field`を実際に実装する
     ——保存済みJSONを`get`で取得し`open_runo_rjson::extract_path`で
     部分抽出して返すだけの薄い実装、`extract_path`自体は完成・
     テスト済みなので配線のみ。現状は`DbBackend`のデフォルト実装
     (未対応エラー)にフォールバックしたままで、これは`RjsonBackend`が
  ...
  4. その後、Phase 3(永続化・レプリケーション・インデックス化、
     `docs/rjson.md`「Phase 3以降」節参照)に着手する場合は、
     `aruaru-db`の複数クレート・複数セッションにわたる規模感を
     参考にスコープを見積もること(1パスで完了する規模ではない)。
  **今回変更したファイル**: `crates/open-runo-rjson/src/lib.rs`・
  `crates/open-runo-db/src/lib.rs`・`docs/rjson.md`(両リポジトリ)。
