...
Rustの Poem 互換フレームワーク(自前実装、パッケージ非依存)との
「スムーズな連携」——ルーティング/ハンドラのAPI形状に互換性を保った
統合——を行うことが固有の役割**。新機能・改善タスクを検討する際は、
この4点(Tauri互換UI・REST API不要・契約不要・自作AI高速化)と、Poem
互換フレームワークとの連携のスムーズさ、の両軸で完成度・利便性・
