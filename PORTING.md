...
>
> 対象バージョン: workspace 0.1.0（18 クレート / 256 テスト）
> 最終更新: 2026-07-11
...
serve_tls}` が使えます。ネイティブのシステムトレイ常駐アプリ・OS
ネイティブ通知・Windows インストーラーが必要な場合は `apps/desktop-tray`
...
```bash
cd open-runo
cargo test --workspace     # 256 テスト + doctest が全部通れば OK
