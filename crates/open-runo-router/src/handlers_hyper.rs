...
/// commit, not the latest value. Delegates to `DbBackend::get_at_commit`,
/// which only `AruaruDbBackend` implements for real (every other backend
/// reports 501, since they have no commit history to query against).
...
mod tests {
    ...
    /// in aruaru-db's own `aruaru-query::engine` test suite, and end-to-end
    /// through this crate's own pgwire client in
    /// `open-runo-db/tests/aruaru_as_of_commit.rs`; this test only proves
    /// this crate's HTTP plumbing degrades honestly for backends that
    /// don't support it.
    #[tokio::test]
